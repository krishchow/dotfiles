...
- **`.zshrc`** sources, in order: `.env` → `.path` → `.aliases` → `.history.zsh` → `.git-exclude.zsh` → `.agent-sync.zsh` → `.runtime-version.zsh` → `.secrets.zsh`. `DOTFILES` is auto-detected from `${0:A:h}` so the repo works regardless of clone location.
